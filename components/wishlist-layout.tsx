...
import RefreshButton from "@/components/cart-content/refresh-button";
      ...
      <LayoutWrapper>
        <div className="flex flex-col items-center justify-center py-32 border-2 border-dashed border-destructive/20 bg-destructive/5">
          ...
          <p className="text-sm font-bold text-muted-foreground mb-10 max-w-sm text-center uppercase tracking-tight">
            تعذر الوصول إلى سجل الأمنيات الخاص بك. يرجى التحقق من استجابة
            الخادم.
          </p>
          <RefreshButton />
        </div>
  ...
  return (
    <LayoutWrapper>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-0 border-t border-r border-foreground/10">
        {items.map((product) => {
          const originalPrice = product.price ? Number(product.price) : null;
          const currentPrice = product.salePrice
            ? Number(product.salePrice)
            : Number(product.price);
          const hasOffer = originalPrice && currentPrice < originalPrice;
          ...
          return (
            ...
            >
              ...
              >
                ...
                />
                {hasOffer && (
                  <div className="absolute top-0 right-0 z-20 bg-primary text-white px-3 py-1 text-[10px] font-black uppercase tracking-widest">
                    SAVE_{savePercent}%
                  </div>
                )}
              </Link>
